...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gotComment'])) {
    ...
    if($email === '' || $comment === '') {
        ...
        $errMsg []= "Комментарий должен быть более 20 символов";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_comment'])) {
    ...
    if($text === '') {
        $errMsg []= 'Комментарий не имеет текста';
    } elseif (mb_strlen($text, 'UTF8') < 20) {
        $errMsg []= "Количество символов в комментарии должно быть более 20";
    } else {
        $com = [
            'comment' => $text,
            'status' => $publish
